...
def consecutive_primes_v2(a, b, set_primes):
    '''Finds the number of consecutive primes that produces the formula
    n^2 + an + b, starting with n = 0.
    '''
    # We already know that b and a + b + 1 are primes, so we start with n = 2
    n, val = 2, 2*2 + a*2 + b
    ...
    while val in set_primes:
        ...
        val = n*n + a*n + b
    # In case val is not prime, n is equal to the number of consecutive primes
    return n
...
def max_cons_primes_v2(bound):
    ...
    for p in list_primes:
        ...
        for p_prime in list_primes:
            ...
            d_primes[(a, b)] = consecutive_primes_v2(a, b, set_primes)
...
if __name__ == '__main__':
    ...
    # print(max_cons_primes(1000))  # (-61, 971), 0.15s
...
(1*)
When n = 0 then n^2 + an + b = b, and this has to be a prime, let's call it p.
Also, when n = 1, n^2 + an + b = 1 + a + p, which is also a prime, let's call
it p'. So, instead of looking for a and b, we can search for p and p'.
