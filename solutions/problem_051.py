...
def find_prime(x, max_digs):
    '''Finds the smallest prime with x primes among the numbers generated by
    replacing part of the number by the same 0-9 number
    '''
...
def apply_mask_to_number(mask, num):
    ...
    returns the number of primes which result from substituting the positions with '*' by the same digit in num
    '''
    count = 0
    for d in range(10):
...
def find_prime_v2(x, limit):
    ...
    for prime in primes:
        ...
        masks = bit_strings(digits, 1)  # ['*XX', 'X*X', 'XX*']
...
if __name__ == '__main__':
    max_digs = 6
    start = perf_counter()
    print(find_prime(8, max_digs))  # 121313, 6.8s
    print(f'Execution time: {perf_counter() - start} seconds')
    '''start = perf_counter()
    print(find_prime_v2(5, 10**max_digs))
    print(f'Execution time: {perf_counter() - start} seconds')'''
    start = perf_counter()
    print(len(sieve_Eratosthenes(1000000)))
    print(f'Execution time: {perf_counter() - start} seconds')
