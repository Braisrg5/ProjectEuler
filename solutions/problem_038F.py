...
def pandigital_multiples_2():
    '''Finds the max 1 to 9 pandigital 9-digit number that can be formed as the
    concatenated products of an integer with n = 2.
    The bounds are explained in (1*).'''
    # We can start with 9183, because any number smaller than that will
    # have a concatenated product less than 918273645
    for x in range(9877, 9183, -1):
...
# (1*)
# For n = 2, since we need to obtain 9 digits, the digits of x and 2x must sum
# to 9. Since x < 2x and we can only increase by one digit each time we
# multiply, we have the following conditions:
#             4 digits: x * 1 ∈ [1234, 9876]
#             5 digits: x * 2 ∈ [12345, 98765] => x ∈ [6173, 49382]
# The intersection of the two intervals gives us x ∈ [6173, 9876]
# x can't be smaller than 9183 because if it were the resulting pandigital
# would be less than 918273645.
...
# The concatenated product for x = 329 would be less than 918273645, no need to
# check n = 3 then.
