...
# difference are pentagonal and D = |Pk-Pj| is minimized; what is the value of D?  # noqa
from resources.useful_functions import is_pentagonal
from tqdm import tqdm


def pent_pair(bound):
    '''Tries to find the difference between the pair of pentagonal numbers for
    which their sum and difference are also pentagonal and D = |Pk-Pj| is
    minimized.'''
    pentagonals = [i*(3*i-1)//2 for i in range(1, bound+3)]
    min_diff = float('inf')
    # We go through all pairs of pentagonal numbers for i, j < bound
    ...
    for k in tqdm(range(bound+2)):
        ...
        for j in range(k-1, 0, -1):
            ...
            if is_pentagonal(pk + pj) and is_pentagonal(new_d):
                min_diff = new_d
                print(f'Pj = {pj}, Pk = {pk}, D = {pk - pj}')
...
# I created the program without a known upper bound, and the first candidate
# that I found was 5482660, which happens to be the PE solution, but I wondered
# how to prove that it is in fact the smallest. After some thought, I concluded
# that we need to compute up to an k such that P(k+1) - P(k) > 5482660, because
# beyond that index, it's impossible for the difference to be smaller.
...
# In other words, if we check up to kMAX = 1827553, we will necessarily find
