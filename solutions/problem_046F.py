...
(1*)
We have a bound N for the primes and the composite numbers, so it wouldn't make
sense to compute numbers ptwicesq that are greater than N, because when we
subtract the ptwicesq from the composite numbers, it would be redundant to have
numbers greater than N in the list. So, for each prime p, we want:
