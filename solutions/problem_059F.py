...
def load_message(path):
    '''Loads the encrypted message in path.'''
...
def sum_ascii_vals(text):
    '''Calculates the sum of the ascii values of a string.'''
    return sum(ord(c) for c in text)


def find_password(path):
    '''Finds the password that decodes the text in path.'''
    # List of common English words to look for in the decrypted text.
    # common_words = ['the', 'be', 'and', 'of', 'a', 'in', 'to']
    # Surprisingly or not, it is enough to consider ' the '.
    common_words = [' the ']
    ...
    for password in possible_passwords:
        ...
        if decrypted:
            ...
            if all(word in decrypted_text.lower() for word in common_words):
                ...
                if answer.lower() == 'y':
                    return sum_ascii_vals(decrypted_text)
    # Some common words not in the text
    return None
