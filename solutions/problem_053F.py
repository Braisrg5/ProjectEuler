...
# In general, nCr = n!/[r!(n-r)!], where r <= n, n! = n*(n-1)*...*2*1 and
# 0! = 1.
...
def combinatorics_greater_v2(bound, ncr_bound, start=1):
    '''Finds the values of nCr for 1 <= n <= bound that are greater than
    ncr_bound. Start can be set to skip values of n that we know don't give a
    combinatoric bigger than ncr_bound.'''
    ...
    for n in range(start, bound+1):
        ...
        current = comb(n, n//2)
        # If the biggest value isn't bigger than ncr_bound, we can skip this n
        if current > ncr_bound:
            n_count += 1
            ...
            for r in range(n//2, 0, -1):
                ...
                # No more values bigger than ncr_bound
                if current <= ncr_bound:
                    break
                n_count += 1
...
# we can see that nCr has a maximum in r1 = floor(n/2) and r2 = ceil(n/2)
# (which can be equal if n is even).

# So, to solve the problem, we can start checking if values exceed one-million
# at nC(r1) and go downwards. We will need to multiply by two the number of
# values we find and adjust for even numbers (r1 == r2).
