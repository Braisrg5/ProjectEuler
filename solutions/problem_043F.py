...
def sum_pandigitals_property():
    '''Sum all numbers with the property explained in the statement.'''
    def recurring_property(num, i):
        '''Finds recursively the pandigital numbers that satisfy the property.
        primes = [2, 3, 5, 7, 11, 13]
        We start with a 3-digit 9 to 1 pandigital number that is divisible by
        17, go through the pandigitals of three digits that are divisible by 13
        checking if the property is satisfied. If it is, we do the same thing
        with 11, and so on. This is done recursively to simplify the code (the
        ...
        if i >= 0:
            ...
            for triad in dict_3[p]:
                # First two digits of the constructed num have to coincide with
                # the last two digits of the new triad
                # Also the new digit can't be in the num (pandigital)
                if num[:2] == triad[1:] and triad[0] not in num:
                    # We call the property updating the constructed num and for
...
# pandigital numbers less than 3 digits that are divisible by 17, and do the
# same for 13, 11, 7, 5, 3 and 2.
# Then, start with one of the divisible by 17, let's call it div17 and go
# through the list of those divisible by 13, checking if the last 2 digits
# coincide with the first 2 of div17. If there is a match, we do the same with
# 11, and so on.
