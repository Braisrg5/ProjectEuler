...
proper divisors of 28 would be 1 + 2 + 4 + 7 = 28, which means that 28
is a perfect number.
A number n is called deficient if the sum of its proper divisors is
less than n and it is called abundant if this sum exceeds n.
As 12 is the smallest abundant number, 1 + 2 + 3 + 4 + 6 = 16, smallest
...
def not_sum_abundants_v3(n):
    ...
    abundants = {i for i in range(1, n+1) if is_abundant(i)}
    non_sum = [
        i for i in range(1, n+1) if not is_sum_abundants(i, abundants)
    ]
    return sum(non_sum)
...
if __name__ == "__main__":
    ...
    # print(not_sum_abundants_v2(28123))  # 4179871, 3.7s
    print(not_sum_abundants_v3(28123))  # 4179871, 0.5s
